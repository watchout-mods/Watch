local MAJOR, MINOR = "LibObjectDocs-1", 1
local Lib = LibStub:NewLibrary(MAJOR, MINOR);
if not Lib then return; end

--- Constant for setters. Setters require one or more arguments and are not safe
-- to call.
local METHOD_TYPE_SETTER = 1;

--- Constant for getters. Getters may require any number of arguments but are
-- safe to call. The API may in the future allow definition of default arguments
-- that yield an exemplary return value.
local METHOD_TYPE_GETTER = 2;

--- Constant for testers. Usually named Is*. Usually safe to call. But some may
-- require arguments (like :IsType(string))
local METHOD_TYPE_TESTER = 3;

--- Constant for actions. Actions like :Play(), :Run(), :StopMovingOrSizing(),
-- etc. They are all unsafe thus will not be called.
local METHOD_TYPE_ACTION = 4;

--- Constant for safe methods that may not be any of the above.
local METHOD_TYPE_SAFE = 8;

--- Constant for unsafe methods that may not be any of the above.
local METHOD_TYPE_UNSAFE = 9;

Lib.METHOD_TYPE_SETTER = METHOD_TYPE_SETTER;
Lib.METHOD_TYPE_GETTER = METHOD_TYPE_GETTER;
Lib.METHOD_TYPE_TESTER = METHOD_TYPE_TESTER;
Lib.METHOD_TYPE_ACTION = METHOD_TYPE_ACTION;
Lib.METHOD_TYPE_SAFE   = METHOD_TYPE_SAFE;
Lib.METHOD_TYPE_UNSAFE = METHOD_TYPE_UNSAFE;
